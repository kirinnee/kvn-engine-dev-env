 ...
 *
 *  MIT License
 */(function(){'use strict';// Setup default properties.
...
y=typeof y!=='number'?self._pos[1]:y;z=typeof z!=='number'?self._pos[2]:z;if(typeof x==='number'){self._pos=[x,y,z];self.ctx.listener.setPosition(self._pos[0],self._pos[1],self._pos[2])}else{return self._pos}return self};/**
...
if(!sound._panner||!sound._panner.pan){setupPanner(sound,pannerType)}if(pannerType==='spatial'){sound._panner.setPosition(pan,0,0)}else{sound._panner.pan.setValueAtTime(pan,Howler.ctx.currentTime)}}self._emit('stereo',sound._id)}else{return sound._stereo}}}return self};/**
...
if(!sound._panner||sound._panner.pan){setupPanner(sound,'spatial')}sound._panner.setPosition(x,y,z)}self._emit('pos',sound._id)}else{return sound._pos}}}return self};/**
...
if(!sound._pos){sound._pos=self._pos||[0,0,-0.5]}setupPanner(sound,'spatial')}sound._panner.setOrientation(x,y,z)}self._emit('orientation',sound._id)}else{return sound._orientation}}}return self};/**
...
return self._pannerAttr}else if(args.length===1){if(_typeof(args[0])==='object'){o=args[0];// Set the grou's panner attribute values.
...
self._orientation=parent._orientation;self._pos=parent._pos;self._pannerAttr=parent._pannerAttr;// Complete resetting of the sound.
...
if(!sound._paused){sound._parent.pause(sound._id,true).play(sound._id)}}})();